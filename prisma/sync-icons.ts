...
// Copy of defaultCategories from lib/default-categories.ts to avoid import issues
...
async function main() {
    console.log('Starting full icon sync...')

    for (const cat of defaultCategories) {
        // Update parent categories (by name, for all users)
        // We use updateMany to target all categories with this name
        ...
        console.log(`Updated ${result.count} instances of category '${cat.name}' -> ${cat.icon}`)

        // Update subcategories
        if (cat.subCategories && cat.subCategories.length > 0) {
            for (const sub of cat.subCategories) {
                const subResult = await prisma.category.updateMany({
                    where: {
                        name: sub.name,
                        // Ensure we are updating subcategories of the correct parent type
                        parent: {
                            name: cat.name
                        }
                    },
                ...
                console.log(`  Updated ${subResult.count} instances of subcategory '${sub.name}' -> ${sub.icon}`)
            }
        }
    }
